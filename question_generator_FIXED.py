...
- Double-check that the correct answer index matches the right option"""
                ...
                try:
                    ...
                    with open(f'/tmp/failed_question_gen_{difficulty}.txt', 'w') as f:
                        f.write(response_text)
                    print(f"Full response saved to /tmp/failed_question_gen_{difficulty}.txt")
                    
                    return jsonify({
                        'error': f'Failed to parse {difficulty} questions. Claude response was not valid JSON.',
                        'detail': str(e),
